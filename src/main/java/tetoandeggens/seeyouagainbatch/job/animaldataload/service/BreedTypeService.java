...
import tetoandeggens.seeyouagainbatch.job.animaldataload.repository.BreedTypeRepository;
...
public class BreedTypeService {
	...
	@Transactional
	public Map<String, Long> processBreedTypes(List<AnimalPublicDataDto> publicDataList) {
		...
		if (!newBreedTypeCodes.isEmpty()) {
			...
			Map<String, AnimalPublicDataDto> codeToDataMap = new HashMap<>();
			for (AnimalPublicDataDto dto : publicDataList) {
			...
			for (String code : newBreedTypeCodes) {
				AnimalPublicDataDto dto = codeToDataMap.get(code);
