package tetoandeggens.seeyouagainbatch.job.abandonedanimaldataload.integration;
...
class AbandonedAnimalDataLoadIntegrationTest extends BatchTestConfig {
	...
	@Autowired
	private Job abandonedAnimalDataLoadJob;
	...
	@Test
	@DisplayName("유기동물 데이터 로드 Job이 정상적으로 실행되어야 한다")
	void shouldCompleteJobSuccessfullyWithValidData() throws Exception {
		when(publicDataApiClient.fetchAbandonedAnimals(
	...
	void shouldCompleteSuccessfullyWithEmptyApiResponse() throws Exception {
		...
		assertThat(countAbandonedAnimals()).isEqualTo(0);
	...
	void shouldProcessMultiplePagesSequentially() throws Exception {
		...
		assertThat(countAbandonedAnimals()).isEqualTo(4);

		verify(publicDataApiClient, times(3)).fetchAbandonedAnimals(
			anyString(), anyString(), anyString(), anyInt(), anyString(), anyInt());
	}

	@Test
	@DisplayName("동일한 보호소의 유기동물이 여러 건 있어도 보호소는 한 번만 저장되어야 한다")
	void shouldSaveCenterLocationOnlyOnce() throws Exception {
		...
		assertThat(countAbandonedAnimals()).isGreaterThan(1);
	}

	private void setupJobLauncherTestUtils() {
		jobLauncherTestUtils.setJob(abandonedAnimalDataLoadJob);
	}
	...
	private int countCenterLocations() {
		return namedParameterJdbcTemplate.getJdbcTemplate()
			.queryForObject("SELECT COUNT(*) FROM center_location", Integer.class);
	}
	...
	private int countAbandonedAnimals() {
		return namedParameterJdbcTemplate.getJdbcTemplate()
			.queryForObject("SELECT COUNT(*) FROM abandoned_animal", Integer.class);
	}

	private int countAbandonedAnimalProfiles() {
		return namedParameterJdbcTemplate.getJdbcTemplate()
			.queryForObject("SELECT COUNT(*) FROM abandoned_animal_profile", Integer.class);
	}

	private AbandonedAnimalApiResponseWrapper createMockApiResponse() {
		List<AbandonedAnimalPublicDataDto> itemList = List.of(
		...
		AbandonedAnimalApiResponseWrapper.Items items = new AbandonedAnimalApiResponseWrapper.Items(itemList);
		AbandonedAnimalApiResponseWrapper.Body body = new AbandonedAnimalApiResponseWrapper.Body(items);
		AbandonedAnimalApiResponseWrapper.Response response = new AbandonedAnimalApiResponseWrapper.Response(body);
		return new AbandonedAnimalApiResponseWrapper(response);
	}

	private AbandonedAnimalApiResponseWrapper createSecondPageMockResponse() {
		List<AbandonedAnimalPublicDataDto> itemList = List.of(
	...
	private AbandonedAnimalApiResponseWrapper createMockApiResponseWithSameCenterLocation() {
		List<AbandonedAnimalPublicDataDto> itemList = List.of(
	...
	private AbandonedAnimalApiResponseWrapper createEmptyApiResponse() {
		AbandonedAnimalApiResponseWrapper.Items items = new AbandonedAnimalApiResponseWrapper.Items(List.of());
		AbandonedAnimalApiResponseWrapper.Body body = new AbandonedAnimalApiResponseWrapper.Body(items);
		AbandonedAnimalApiResponseWrapper.Response response = new AbandonedAnimalApiResponseWrapper.Response(body);
		return new AbandonedAnimalApiResponseWrapper(response);
	}

	private AbandonedAnimalPublicDataDto createTestAnimalDto(
