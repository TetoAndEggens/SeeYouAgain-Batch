package tetoandeggens.seeyouagainbatch.job.keywordnotification.integration;
...
@DisplayName("KeywordNotification 통합 테스트")
class KeywordNotificationIntegrationTest extends BatchTestConfig {
	...
	@Autowired
	private Job keywordNotificationJob;
	...
	@Test
	@DisplayName("키워드 알림 Job이 정상적으로 실행되어야 한다")
	...
	@Test
	@DisplayName("품종 키워드에 대한 알림을 전송해야 한다")
	void shouldSendNotificationForBreedKeyword() throws Exception {
		JobExecution jobExecution = jobLauncherTestUtils.launchJob(jobParameters);
	...
	@Test
	@DisplayName("지역 키워드에 대한 알림을 전송해야 한다")
	void shouldSendNotificationForLocationKeyword() throws Exception {
		JobExecution jobExecution = jobLauncherTestUtils.launchJob(jobParameters);
	...
	@Test
	@DisplayName("해당 날짜의 동물에 대한 알림만 전송해야 한다")
	void shouldSendNotificationOnlyForSpecificDate() throws Exception {
		...
		verify(fcmService, never()).sendMulticastNotification(
			anyList(), anyString(), anyString(), anyString(), any(), any(), anyString(), anyString()
		);
	}

	@Test
	@DisplayName("FCM 토큰이 없으면 알림을 전송하지 않아야 한다")
	void shouldNotSendNotificationWhenNoFcmTokens() throws Exception {
	...
	@Test
	@DisplayName("구독 키워드가 없으면 Job이 정상 완료되어야 한다")
	...
	@Test
	@DisplayName("매칭된 동물이 없으면 Job이 정상 완료되어야 한다")
	void shouldCompleteSuccessfullyWithNoMatchedAnimals() throws Exception {
		namedParameterJdbcTemplate.getJdbcTemplate().execute("DELETE FROM animal_by_keyword");
	...
	@Test
	@DisplayName("푸시 알림 비활성화 사용자에게는 알림을 전송하지 않아야 한다")
	void shouldNotSendNotificationToDisabledPushUsers() throws Exception {
		namedParameterJdbcTemplate.getJdbcTemplate().execute(
			"UPDATE member SET is_push_enabled = false WHERE member_id = 1");
	...
	@Test
	@DisplayName("삭제된 사용자에게는 알림을 전송하지 않아야 한다")
	void shouldNotSendNotificationToDeletedUsers() throws Exception {
		namedParameterJdbcTemplate.getJdbcTemplate().execute(
	...
	@Test
	@DisplayName("대량의 키워드 알림도 청크 단위로 정상 처리되어야 한다")
	...
	@Test
	@DisplayName("동일 키워드에 대한 알림을 그룹화하여 전송해야 한다")
	void shouldGroupNotificationsByKeyword() throws Exception {
		namedParameterJdbcTemplate.getJdbcTemplate().execute("SET FOREIGN_KEY_CHECKS = 0");
		namedParameterJdbcTemplate.getJdbcTemplate().execute("DELETE FROM animal_by_keyword");
		namedParameterJdbcTemplate.getJdbcTemplate().execute("DELETE FROM notification_keyword");
		namedParameterJdbcTemplate.getJdbcTemplate().execute("DELETE FROM fcm_token");
		...
		namedParameterJdbcTemplate.getJdbcTemplate().execute(
			"INSERT INTO fcm_token (fcm_token_id, token, member_id, created_at, updated_at) VALUES " +
				"(101, 'token101', 1, NOW(), NOW()), " +
				"(102, 'token102', 2, NOW(), NOW())");
		...
		namedParameterJdbcTemplate.getJdbcTemplate().execute(
			"INSERT INTO animal (animal_id, animal_type, desertion_no, happen_date, city, town, color, birth, weight, " +
				...
				"(102, 'ABANDONED', 'TEST-102', '2025-01-15', '서울특별시', '송파구', '흰색', '2021(년생)', '4(Kg)', " +
				"'NOTICE-102', '2025-01-15', '2025-01-30', '보호중', 'F', 'N', '활발', 'DOG', 1, 1, false, NOW(), NOW())");

		namedParameterJdbcTemplate.getJdbcTemplate().execute(
			"INSERT INTO animal_by_keyword (animal_by_keyword_id, notification_keyword_id, animal_id, created_at, updated_at) VALUES " +
				"(101, 101, 101, NOW(), NOW()), " +
				"(102, 101, 102, NOW(), NOW()), " +
				"(103, 102, 101, NOW(), NOW()), " +
				"(104, 102, 102, NOW(), NOW())");
	...
	private void setupJobLauncherTestUtils() {
		jobLauncherTestUtils.setJob(keywordNotificationJob);
	}
	...
	private void setupTestData() {
		...
		namedParameterJdbcTemplate.getJdbcTemplate().execute(
			"INSERT INTO fcm_token (fcm_token_id, token, member_id, created_at, updated_at) VALUES " +
				"(1, 'test-token-001', 1, NOW(), NOW()), " +
				"(2, 'test-token-002', 2, NOW(), NOW())");
	...
	private void setupLargeTestData(int animalCount, int keywordCount) {
		...
		insertLargeTestAnimalByKeyword(animalCount, keywordCount);
	}
	...
	private void insertLargeTestAnimalByKeyword(int animalCount, int keywordCount) {
		StringBuilder sql = new StringBuilder(
			"INSERT INTO animal_by_keyword (animal_by_keyword_id, notification_keyword_id, animal_id, created_at, updated_at) VALUES ");

		int id = 1;
		for (int k = 1; k <= keywordCount; k++) {
			for (int a = 1; a <= animalCount; a++) {
				if (id > 1) {
					sql.append(",");
				}
				sql.append(String.format("(%d, %d, %d, NOW(), NOW())", id, k, a));
				id++;
			}
		}
